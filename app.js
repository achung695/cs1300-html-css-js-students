...
const updatePage = async () => {
  ...
  const filteredFruitsArray = fruitsArray.filter((fruit, i) => {
    return fruit.nutritions.sugar < 15;
  })
  console.log(filteredFruitsArray);
  // TODO: Create a new HTML element to display your data

  const mappedFruitsArray = filteredFruitsArray.map(element => {
    return element.name
  })

  console.log(mappedFruitsArray)

 
  const newElement = document.createElement('div');
  newElement.innerHTML = mappedFruitsArray;
